...
df["NewDate"] = [str(date.year)+"-"+str(date.month) for date in df["CreateDate"]]
df["ID"]=df["UserId"].astype(str)+"_"+df["NewDate"].astype(str)
...
new_df=pd.pivot_table(df,values="UserId",index="ID",columns="Service")
new_df=new_df+1
new_df=new_df.fillna(0)
new_df=new_df.applymap(lambda x:1 if x>0 else 0)
new_df.head()

###Second options for creating
alt_df = df.groupby(['ID', 'Service'])['Service'].count().unstack().fillna(0).applymap(lambda x: 1 if x > 0 else 0)
alt_df.head()
# Adım 2: Birliktelik kurallarını oluşturunuz.
from mlxtend.frequent_patterns import apriori, association_rules
aprior_df=apriori(new_df,
                  min_support=0.005,
                  use_colnames=True,
                  low_memory=True)


rules = association_rules(aprior_df,
                          metric="support",
                          min_threshold=0.005)
...
###Func for this codes blogs
def recommendation_func(df,product_id,th=0.01):
    new_df=df.groupby(["ID","Service"])["Service"].count().unstack().fillna(0).applymap(lambda x:1 if x>0 else 0)
    aprior_df=apriori(new_df,
                      min_support=th,
                      use_colnames=True,
                      low_memory=True)
...
rec_list_2=recommendation_func(df,"2_1",th=0.005)
print(rec_list)
print(rec_list_2)